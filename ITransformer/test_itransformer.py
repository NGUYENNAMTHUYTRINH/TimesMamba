#!/usr/bin/env python3
"""
Testing script for ITransformer model
...
def test_model():
    ...
    if not os.path.exists(model_path):
        ...
        print("Please run train_itransformer.py first")
        return
    ...
    print(f"\nTesting ITransformer on {dataset}...")
    with torch.no_grad():
    ...
    print("\n" + "=" * 60)
    print("TEST RESULTS - ITransformer")
    print("=" * 60)
    ...
    plt.savefig(f'figures/test_results_ITransformer_{dataset}.png', dpi=100, bbox_inches='tight')
    print(f"Test plot saved: figures/test_results_ITransformer_{dataset}.png")
    plt.close()
    ...
    pred_df.to_csv(f'predictions/results_ITransformer_{dataset}.csv', index=False)
    print(f"Results saved: predictions/results_ITransformer_{dataset}.csv")
